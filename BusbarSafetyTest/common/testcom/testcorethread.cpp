...
bool TestCoreThread::startTest(sTestDataItem &item,QString & recv , const QString &test , int step , int &stepTotal)
{
    QString sendStr = "";
//    mStep = 1;
...
bool TestCoreThread::testGND( QString & recv)//acw
{
    bool ret = false;
    sTestDataItem item;
    QString sendStr = "";
...
bool TestCoreThread::testIR(QString & recv)
{
    bool ret = false;
    sTestDataItem item;
    QString sendStr = "";
...
bool TestCoreThread::testACW(QString & recv)
{
    bool ret = false;
    sTestDataItem item;
    QString sendStr = "";
...
void TestCoreThread::run()
{
    ...
    QString recv = "";
    sSnItem *sn = &(mItem->sn);
    if(sn->isIr)testIR(recv);
    if(sn->isAcw)testACW(recv);
    if(sn->isGnd)testGND(recv);
    emit overSig();
}
