...
void Json_Pack::head(QJsonObject &obj)
{
    ...
    obj.insert("Gnd", mPro->gnd);//mPro->gnd
    obj.insert("Ir", mPro->ir);
    obj.insert("Acw", mPro->acw);
    obj.insert("Dcw", mPro->dcw);
...
QString Json_Pack::http_get(const QString &method, const QString &ip, int port)
{
    ...
    http.get(url.arg(ip).arg(port).arg(method))
        .header("content-type", "application/json")
        .onSuccess([&](QString result) {qDebug()<<"result   "<< result; mac = result;emit httpSig(result,true);})
        .onFailed([&](QString error) {qDebug()<<"error   "<< error;emit httpSig(error,false); })
